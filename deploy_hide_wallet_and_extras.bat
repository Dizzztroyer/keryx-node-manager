...
git add -A >> "%LOG%" 2>&1
git commit -m "0.2.7: hide wallet card behind feature flag, add model tier reference table to Models page (verified against keryx-miner/keryx-node source), flag implausible per-device hashrate on Dashboard, write full-wallet-integration research plan" >> "%LOG%" 2>&1
git push >> "%LOG%" 2>&1
