...
git add -A >> "%LOG%" 2>&1
git commit -m "0.2.7: surface keryx-miner's CUDA runtime failure on the Dashboard with a direct download link, instead of only in Logs" >> "%LOG%" 2>&1
git push >> "%LOG%" 2>&1
